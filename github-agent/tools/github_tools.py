...
def search_issues(client: GitHubClient, query: str, limit: int = 20) -> ToolResult:
    """Search issues using GitHub search syntax (text queries only, e.g. 'is:issue is:open label:bug repo:owner/repo').
    Do NOT use for date-range filtering or relative time queries — use a synthesized tool for that instead."""
    calls_before = client.api_calls
