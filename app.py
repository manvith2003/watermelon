    ...
    if run_btn and instruction:
        if not os.environ.get("GITHUB_TOKEN"):
            ...
            for step in report["steps"]:
                ...
                memory_note_html = ""
                if step.get("memory_note"):
                    memory_note_html = f'<br><span class="memory-note">🧠 {step["memory_note"]}</span>'

                error_html = ""
                if step.get("error"):
                    error_html = f'<br><span style="color:#dc3545">⚠️ {step["error"][:150]}</span>'
                    if step.get("decision"):
                        error_html += f'<br><span style="color:#856404">Agent decided: {step["decision"]}</span>'
